...
where node >nul 2>&1
if errorlevel 1 (
    echo          %RED%[FAIL] Not found. Install from https://nodejs.org%R%
    echo.
    pause & exit /b 1
)
...
if not exist "%ROOT%\.db_imported" (
    if exist "%ROOT%\panda.sql" (
        ...
        echo          %GRN%[OK]%R%  panda.sql imported
    )
)
echo.
...
echo          %GRN%[OK]%R%  PHP packages installed

php artisan key:generate --force >nul 2>&1
echo          %GRN%[OK]%R%  APP_KEY generated
...
if errorlevel 1 (
    echo.
    echo          %RED%[FAIL] Run manually: cd frontend-react ^&^& npm install%R%
    echo.
    popd & pause & exit /b 1
)
echo          %GRN%[OK]%R%  Node packages installed
popd

echo.
echo   %GRY%  --------------------------------------------------%R%
echo   %GRN%  [OK] Setup complete!%R%
echo   %GRY%  --------------------------------------------------%R%
echo.
...
where php >nul 2>&1
if errorlevel 1 ( echo   %RED%[FAIL]%R%  PHP not found & pause & exit /b 1 )
for /f "tokens=*" %%v in ('php -r "echo phpversion();" 2^>nul') do set "PHPV=%%v"
echo   %GRN%[OK]%R%  PHP       %GRY%v%PHPV%%R%

where node >nul 2>&1
if errorlevel 1 ( echo   %RED%[FAIL]%R%  Node.js not found & pause & exit /b 1 )
for /f "tokens=*" %%v in ('node -v 2^>nul') do set "NODEV=%%v"
echo   %GRN%[OK]%R%  Node.js   %GRY%%NODEV%%R%
